...
trait GoogleTranslateComponent extends TranslateComponent {
  this: HttpClientFactoryComponent =>

  class GoogleTranslate extends Translate with Logging {
    ...
    def identifyLang(text: String): Option[String] = {
      extractJsonField(detectUrl + Helpers.urlEncode(text), "language")
    }
